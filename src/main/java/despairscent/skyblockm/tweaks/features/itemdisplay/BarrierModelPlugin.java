...
public class BarrierModelPlugin implements ModelLoadingPlugin {
    ...
    public void initialize(Context pluginContext) {
        pluginContext.modifyModelAfterBake().register((model, context) -> {
            if (context.topLevelId() != null && context.topLevelId().id().getNamespace().equals("minecraft") && context.topLevelId().id().getPath().equals("barrier") && !"inventory".equals(context.topLevelId().getVariant())) {
                return new BarrierBakedModel(model);
            }
            return model;
        });
    ...
    public void onInitializeModelLoader(Context pluginContext) {
        pluginContext.modifyModelAfterBake().register((model, context) -> {
            ...
            if (context.id() != null && context.id().getNamespace().equals("minecraft") && (context.id().getPath().equals("barrier") || context.id().getPath().equals("block/barrier"))) {
                if (context.id() instanceof ModelIdentifier modelId && "inventory".equals(modelId.getVariant())) {
                    return model;
                }
                return new BarrierBakedModel(model);
            }
