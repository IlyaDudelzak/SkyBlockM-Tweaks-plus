...
public class CaptchaClassifier {
     ...
     */
    public static ClassificationResult classifyWithConfidence(byte[] colors) {
        ...
        try {
            float[] rgbData = new float[3 * 128 * 128];
            int rOffset = 0;
            int gOffset = 128 * 128;
            int bOffset = 2 * 128 * 128;

            for (int i = 0; i < 128 * 128; i++) {
                int colorByte = colors[i] & 0xFF;
                int baseIndex = colorByte / 4;
                int shadeIndex = colorByte % 4;

                int[] rgb = (baseIndex < BASE_COLORS.length) ? BASE_COLORS[baseIndex] : BASE_COLORS[0];
                float multiplier = MULTIPLIERS[shadeIndex];

                float rNorm = ((rgb[0] * multiplier) / 255.0f - MEAN[0]) / STD[0];
                float gNorm = ((rgb[1] * multiplier) / 255.0f - MEAN[1]) / STD[1];
                float bNorm = ((rgb[2] * multiplier) / 255.0f - MEAN[2]) / STD[2];

                rgbData[rOffset + i] = rNorm;
                rgbData[gOffset + i] = gNorm;
                rgbData[bOffset + i] = bNorm;
            }

            long[] shape = new long[]{1, 3, 128, 128};
            FloatBuffer buffer = FloatBuffer.wrap(rgbData);
            ...
            try (OnnxTensor inputTensor = OnnxTensor.createTensor(env, buffer, shape)) {
                ...
                try (OrtSession.Result result = session.run(Collections.singletonMap(inputName, inputTensor))) {
                    ...
                    float exp2 = (float) Math.exp(logits[2] - maxLogit);
                    float sumExp = exp0 + exp1 + exp2;

                    float[] probs = new float[]{exp0 / sumExp, exp1 / sumExp, exp2 / sumExp};
                    ...
                    CaptchaClass detectedClass = switch (bestClassIndex) {
                    ...
                    return new ClassificationResult(detectedClass, maxProb, logits);
                }
            }
        } catch (Throwable t) {
            System.err.println("[CaptchaClassifier] Inference error: " + t.getMessage());
            t.printStackTrace();
            return null;
        }
    }
